---
layout: default
---

<div style="display: flex; justify-content: space-between; align-items: center;">
  <div style="flex: 1; text-align: left;">
    <!-- 图片部分 -->
    <img title="2020" alt="Alt text" src="assets/img/Me.jpg" style="width:270px;">
  </div>
  <div style="flex: 1; text-align: left; padding-left: 10px;">
    <!-- 文字部分 -->
    <p>
      庄麟升，字明瑞，楚之夷陵人也。号纸檀居士，斋署星霜阁。
    </p>
    <p>
      时人有闲极者，辑为《纸檀先生传》云：先生闲散人也。好读书，善属文。少尝自比管乐。弱冠辞乡，寄迹江湖凡十载有奇。及长，喟然无所成也，遂绝意荣达。赁居危楼仄室，号星霜阁，以其草木星霜之变，于方寸间自有大观，偶作文章以自娱。
    </p>
  </div>
</div>

Welcome to Linsheng's personal site. Here is my [CV (2025)]({{ site.url }}/assets/doc/cv/cv2025.pdf).

## Biography

Currently, I am a postdoctoral researcher at Center for Intelligent Computing,
Greater Bay University, focusing on intelligent constraint programming (CP)
algorithms and solver development.

I earned my Ph.D. in Operations Research from the National University of
Singapore in 2025. Earlier, I received a Master’s (2020) and a Bachelor’s (2017)
degree in Economics from Peking University.

## Research Interests

My research interest is broad, including stochastic modeling, optimization theory and algorithms.

## Paper

Zhuang, Linsheng and Chen, Zhi and Keppo, Jussi, The Impact of Tie Intervals on Rank-Based Contests (August 19, 2024). Available at SSRN: [https://ssrn.com/abstract=4930136](https://ssrn.com/abstract=4930136).

## Contact

- Email: linsheng.z@u.nus.edu / zhuanglinsheng@outlook.com

<br>
