...
**Def. 5.1 (Fejér Monotonicity)** Let $C$ be a nonemoty subset of $\mathcal{H}$ and let $(x_n)$ be a sequence in $\mathcal{H}$. Then $(x_n)$ is Fejér monotone w.r.t. $C$ if $\|x_{n+1}-x\| \le \|x_n-x\|$ for all $x\in C$ and for all $n\in N$. 

**Example 5.2** A bounded increasing sequence $(x_n)$ is Fejér Monotone w.r.t. the set $(\sup x_n, +\infty)$. 

**Definition (Quasi-nonexpansiveness)** For any $\emptyset\not=D\subset\mathcal{H}$, an operator $T:D\to D$ with $\text{Fix}(T)\not=\emptyset$ is [quasi-nonexpansive](https://www.cambridge.org/core/services/aop-cambridge-core/content/view/S144678870001123X) if $\|Tx-p\| \le \|x-p\|$ for all $x\in D$ and $p\in\text{Fix}(T)$. 

**Note.** Non-expansiveness implies quasi-non-expansiveness: $\|Tx - Tp\| \le \|x-p\|$ $\Rightarrow$ $\|Tx - p\| \le \|x - p\|$ for all $p\in\text{Fix}(T)$. 

**Example 5.3** Let $\emptyset\not=D\subset\mathcal{H}$ and $T:D\to D$ be a quasi-nonexpansive operator such that $\text{Fix}(D)\not=\emptyset$. Then $(x_n)$ generated by the fixed point iterations of $T$ is Fejér Monotone. 
...
***Proof.*** By contradiction. If $d_C(x_{n+1}) > d_C(x_n)$ for some $n\in N$, then $\|x_{n+1}-P_C(x_n)\| \ge \|x_{n+1}-P_C(x_{n+1})\| > \|x_n-P_C(x_n)\|$. 
