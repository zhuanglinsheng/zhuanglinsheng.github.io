...
For any $d\in K$, letting $y_t=(1-t)\Pi_K(z)+td$ for any $t\in[0,1]$, then $\|z-y_t\|^2\ge\|z-\Pi_K(z)\|^2$ since $y_t\in K$ by the convexity of $K$. Thus, 
...
**Lemma 3. (Regularity of Fréchet differential)** If the Fréchet derivative of $h:\mathcal{X}\to\mathcal{Y}$ at $\bar{x}$ is onto, then there exists a $M>0$ such that for all $y\in\mathcal{Y}$ there exists a $d\in\mathcal{X}$ with $y = h'(\bar{x})d$ and $\|d\| \le M\|y\|$. 
...
Then it is easy to verify that there exists $M>0$ such that $\|d_1\| \le M\|y\|$. 
...
$$
\limsup_{t\to t_0} A_t = \{y\in\mathcal{Y} : \exists t_k\to t_0, \exist y_k\in A_{t_k} s.t. y_k\to y\}\\
\liminf_{t\to t_0} A_t = \{y\in\mathcal{Y} : \forall t_k\to t_0, \exist y_k\in A_{t_k} s.t. y_k\to y\}
$$
...
(1) For any sequence $(a_n)\subset\bar{A_t}(t_0)$ with $a_n\to a$, since $a_n\in $,\bar{A_t}(t_0) we know $\exist (t^n_k)$ with $t^n_k\to t_0$ s.t. $\exist y^n_k\in A_{t^n_k}$ with $y^n_k\to a_n$. For ant fixed $n$, define 
...
Let $t_n = t^n_{k_n}$ and $y_n = y^n_{k_n} \in A_{t_n}$. We have $t_n\to t_0$ and $\|y_n - a\| \le \|y_n-a_n\| + \|a_n-a\| \to 0$. Thus, $a\in\bar{A_t}(t_0)$ and $\bar{A_t}(t_0)$ is closed. 

(2) For any sequence $(a_n)\subset \underline{A_{t}}(t_0)$ with $a_n\to a$ and for any sequence $t_k\to t_0$, since $a_n\in\underline{A_{t}}(t_0)$ we know $\exist y^n_k\in A_{t_k}$ with $y^n_k\to a_n$. Let $k_1 = 1$. For $n\ge2$, define a sequence $(k_n)$ by 
...
$$
R_D(y) = \{d\in\mathcal{Y}\ :\ \exist t^*>0 \text{ s.t. } y+td\in D, \forall t\in [0,t^*]\ \}
$$
...
T_D(y) \equiv \limsup_{t\downarrow 0}\frac{D-y}{t} = \{d\in\mathcal{Y}:\exist t_k\downarrow0, D_\mathcal{Y}(y+t_kd,D)=o(t_k)\}
$$
...
"$\subset$". By definition of "limsup" we know for all $d\in T_D(y)$, $\exists$ $(t_k)$ with $t_k\downarrow 0$ and $\exist d_k\in \frac{D-y}{t_k}$ such that $d_k\to d$, meaning that $y+t_kd_k\in D$ with $d_k\to d$. Thus, we have 
...
"$\supset$". For all $d\in\mathcal{Y}$ such that $\exist (t_k)$ with $t_k\downarrow 0$ and $D_\mathcal{Y}(y+t_kd,D)=o(t_k)$, meaning that $\exist (x_k)\subset D$ with 
...
Let $d_k = \frac{x_k-y}{t_k}\in\frac{D-y}{t_k}$ and we have $t_k\cdot \|d-d_k\| = o(t_k)$, which implies $d_k\to d$. Thus, $d\in T_D(y)$. 
...
(2) It is obviously that $T^i_D(y)\subset T_D(y)$. Now we want to show that $R_D(y)\subset T^i_D(y)$. For any $d\in R_D(y)$, by definition we know there exists $t^*>0$ such that $y+td\in D$ for all $t\in [0,t^*]$. For any $(t_k)$ with $t_k\downarrow 0$, there exists $K>0$ such that $t_k < t^*$ when $k>K$. Then we have $y+t_kd\in D$, and $D_\mathcal{Y}(y+t_kd,D) = 0 = o(t_k)$. Thus, we conclude that $d\in T^i_D(y)$. 
...
(1) The "$\subset$" direction is straightforward. Now we want to show the "$\supset$" direction. For any $d\in \cup_{t>0}(D-y)/t$ there exists $t^*>0$ and $s\in D$ such that $d = (s-y)/t^*$. Then for all $t\in[0,t^*]$ we have 


$$
y + td = y + t\frac{s-y}{t^*} = (1-t/t^*) \cdot y + t/t^* \cdot s \in D
$$
...
implies that $\bar{\mu}\in\mathcal{K}^*$. 

**Example 1. (Bouligand cone of OP)** Let $\mathcal{K} = \{0\}^m\times R^q_+$ and $G(x)=\begin{bmatrix}h(x)\\g(x)\end{bmatrix}$, the Bouligand cone $T_\mathcal{K}(G(\bar{x}))$ is 
...
**Proposition 2. (RCQ at feasible $\bar{x}$ for solid $\mathcal{K}$)** Assume $G(\bar{x})\in\mathcal{K}$ and $\mathcal{K}$ has a nonempty interior, RCQ is equivalent to $G(\bar{x})+G'(\bar{x})d\in int(\mathcal{K})$ for some $d\in\mathcal{X}$. 
...
"$\Rightarrow$". Since RCQ holds, we know there exists $\epsilon_1>0$ and $\epsilon_2>0$ such that for all $\|y\|\le \epsilon_1$ and $\|z\|\le\epsilon_2$ there exists $d\in\mathcal{X}$ with 
...
RCQ holds if there exists $\epsilon_1>0$ and $\epsilon_2>0$ such that for all $\|y\|\le \epsilon_1$ and $\|z\|\le\epsilon_2$ there exists $d\in\mathcal{X}$ with 
...
Let $G(x) = \begin{bmatrix}h(x)\\g(x)\end{bmatrix}\in R^{m+q}$. By Proposition 1(b) we have RCQ is equivalent to 
...
**Note. (RCQ holds at $\bar{x}$ $\Rightarrow$ $\mathcal{M}(\bar{x})$ is bounded)** If $\mathcal{M}(\bar{x})$ is unbounded then there exists a sequence $(\mu^k)\subset\mathcal{M}(\bar{x})$ with $1/\|\mu^k\|\to0$ and $\mu^k/\|\mu^k\|\to\mu\not=0$. Then we have (1) $-\mu^k\in\mathcal{N}_\mathcal{K}(G(\bar{x}))$ $\Rightarrow$ $-\mu^k/\|\mu^k\|\in\mathcal{N}_\mathcal{K}(G(\bar{x}))$ $\Rightarrow$ $-\mu\in\mathcal{N}_\mathcal{K}(G(\bar{x}))$ by the closeness of normal cone, and (2) 
...
Letting $k\to\infty$ we have $\nabla G(\bar{x})\mu=0$ $\Leftrightarrow$ $\mu\in[G'(\bar{x})\mathcal{X}]^\perp$. Then (1) and (2) contradict to Proposition 1 (c). END. 
...
It's obvious that $\bar{N}$ is a convex cone. Now we want to show its closeness. For any $(z^k)\subset\bar{N}$ that converges to $z$, there exists a sequence $(\mu^k)$ with $-\mu^k\in \mathcal{N}_\mathcal{K}(G(\bar{x}))$ and $z^k=\nabla G(\bar{x})\mu^k$ for all $k$. Case $A)$ If $(\mu^k)$ is bounded then there must be a sub-sequence $(\mu^{i_k})$ that converges to $\mu\in-\mathcal{N}_\mathcal{K}(\bar{x})$ by the closeness of normal cone. Then we must have $z^{i_k}\to z'$ with $z'=\nabla G(\bar{x})\mu\in\bar{N}$. Since $z^k\to z$ we know $z'=z$. Thus $z\in\bar{N}$. Case $B)$ If $(\mu^k)$ is unbounded then there must be a subsequence $(\mu^{i_k}/\|\mu^{i_k}\|)$ with $1/\|\mu^{i_k}\|\to0$ that converges to $\mu\in-\mathcal{N}_\mathcal{K}(\bar{x})$ with $\mu\not=0$ by the closeness of normal cone. Note that $(z^k)$ is bounded since it converges, we know $z^{i_k}/\|\mu^{i_k}\|$ = $\nabla G(\bar{x})\mu^{i_k}/\|\mu^{i_k}\|\to0$ $\Leftrightarrow$ $\nabla G(\bar{x})\mu=0$ $\Leftrightarrow$ $\mu\in[G'(\bar{x})\mathcal{X}]^\perp$. That contradicts to Robinson's CQ by Proposition 1 (c). Thus, $\bar{N}$ is closed. Finally, $\bar{N}$ is a convex closed cone. 
