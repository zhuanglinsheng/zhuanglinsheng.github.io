...
(2) It is obviously that $T^i_D(y)\subset T_D(y)$. Now we want to show that $R_D(y)\subset T^i_D(y)$. For any $d\in R_D(y)$, by definition we know there exists a $t^*>0$ such that $y+td\in D$ for all $t\in [0,t^*]$. For any $(t_k)$ with $t_k\downarrow 0$, there exists $K>0$ such that $t_k < t^*$ when $k>K$. Then we have $y+t_kd\in D$, and $D_\mathcal{Y}(y+t_kd,D) = 0 = o(t_k)$. Thus, we conclude that $d\in T^i_D(y)$. 
