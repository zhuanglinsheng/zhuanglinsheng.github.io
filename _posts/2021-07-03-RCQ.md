...
(1) $\mu^k\in-\mathcal{N}_{\mathcal{K}}(G(\bar{x}))$ which implies that $-\mu^k/\|\mu^k\|\in\mathcal{N}_\mathcal{K}(G(\bar{x}))$, and then $-\mu\in\mathcal{N}_\mathcal{K}(G(\bar{x}))$ by the closeness of normal cone, and 
