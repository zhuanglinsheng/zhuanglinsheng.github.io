...
(1) $\mu^k\in-\mathcal{N}_{\mathcal{K}}(G(\bar{x}))$. It implies that $\frac{\mu^k}{\left\|\mu^k\right\|}\in-\mathcal{N}_{\mathcal{K}}(G(\bar{x}))$, and then $\mu\in-\mathcal{N}_{\mathcal{K}}(G(\bar{x}))$ by the closeness of normal cone. 
...
Case B, if $(\mu^k)$ is unbounded then there must be a subsequence $(\mu^{i_k}/\left\|\mu^{i_k}\right\|)$ with $1/\left\|\mu^{i_k}\right\|\to0$ that converges to $\mu\in-\mathcal{N}_\mathcal{K}(\bar{x})$ with $\mu\not=0$ by the closeness of normal cone. Note that $(z^k)$ is bounded since it converges, we know 
